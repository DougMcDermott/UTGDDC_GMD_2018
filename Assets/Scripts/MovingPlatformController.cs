...
public class MovingPlatformController : RaycastController {
	...
	Vector3 CalculatePlatformMovement () {
		 ...
		 * percentBetweenWaypoints is divided by distance to ensure that the percentage will increase more slowly the further the waypoints are
		 * Calculate new percentBetweenWaypoints with ease function */
	...
	void CalculatePassengerMovement (Vector3 velocity) {
		...
		if (velocity.y != 0) {
			...
			for (int i = 0; i < verticalRayCount; i++) {
				Vector2 rayOrigen = (directionY == -1)?rayOrigen = raycastOrigens.botLeft:rayOrigen = raycastOrigens.topLeft;

				rayOrigen += Vector2.right * (verticalRaySpacing * i);
				RaycastHit2D hit = Physics2D.Raycast (rayOrigen, Vector2.up * directionY, rayLength, passengerMask);
		...
		if (velocity.x != 0) {
			...
			for (int i = 0; i < horizontalRayCount; i++) {
				...
				RaycastHit2D hit = Physics2D.Raycast (rayOrigen, Vector2.right * directionX, rayLength, passengerMask);
